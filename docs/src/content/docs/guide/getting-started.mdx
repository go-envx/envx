...
For each namespace, create a base `<namespace>.yaml` that declares every key the
namespace uses. Give shared keys their default value, and leave environment-specific
values — like a database host — empty to fill in per environment (step 3). Shared
namespaces nest their keys under the namespace name; each app keeps its own config
flat:
...
- A base declares every key the namespace uses, so its full shape lives in one
  place. Keys with a sensible default (`port`, `timeout`) get one here;
  environment-specific values (`host`, `url`) are left empty and set per environment.
...
- Each overlay supplies only what changes, so in `development` just `DATABASE_HOST`,
  `GATEWAY_URL`, and `LOG_LEVEL` come from these files; every other key resolves
  from its base.
- Because `host` and `url` have no base default, each environment sets them in its
  own overlay.

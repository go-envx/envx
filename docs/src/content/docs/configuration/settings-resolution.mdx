...
setting always wins over the matching global setting — for example, a project that
sets `prefix: web` overrides a global `prefix: app`. The example below shows this.
...
The workspace below sets `prefix: app` globally and overrides it to `prefix: web`
on the `web` project. Watch each layer take over in turn — the project setting,
then an `ENVX_PREFIX` variable, then a `--prefix` flag — by the prefix applied to
every resolved key:

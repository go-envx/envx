---
title: get
description: Resolve a project's environment and print a single value.
sidebar:
  order: 3
---

import ExampleBox from '#components/ExampleBox.astro';

```sh
envx get <project> <key>
```

`get` resolves the merged environment for a project and prints the value of a single key. The key is matched case-insensitively, so `HOST` and `host` resolve to the same value.

## Arguments

| Argument | Description |
| --- | --- |
| `<project>` | The project to resolve, as declared in `envx.yaml`. |
| `<key>` | The key to print (case-insensitive). |

## Examples

The examples below resolve a small workspace — an `api` project with one nested `db` namespace and development and production overlays:

  <ExampleBox>
    <Fragment slot="manifest">
      ```yaml
      # envx.yaml
      environments:
        - development
        - production

      projects:
        api:
          includes:
            - db
      ```
    </Fragment>
    <Fragment slot="files">
      ```yaml
      # db.yaml
      db:
        host: # set per environment
        port: 5432
        pool_size: 10
      ```

      ```yaml
      # db.development.yaml
      db:
        host: localhost
      ```

      ```yaml
      # db.production.yaml
      db:
        host: prod.internal
        pool_size: 50
      ```
    </Fragment>
    <Fragment slot="commands">
      ```sh
      # development is the default env.
      envx get api DB_HOST
      # localhost

      # Keys match case-insensitively.
      envx get api db_host
      # localhost

      # Resolve a specific environment.
      envx get api DB_HOST \
        --env production
      # prod.internal
      ```
    </Fragment>
  </ExampleBox>

## Flags

| Flag | Environment variable | Description |
| --- | --- | --- |
| `--env`, `-E` | `ENVX_ENV` | Target environment to resolve. |
| `--require-overlays` | `ENVX_REQUIRE_OVERLAYS` | Require every overlay file to exist. |
| `--prefix` | `ENVX_PREFIX` | Prefix prepended to every key. |
| `--suffix` | `ENVX_SUFFIX` | Suffix appended to every key. |
| `--delimiter` | `ENVX_DELIMITER` | String used to join list values. |
| `--namespace-prefix` | `ENVX_NAMESPACE_PREFIX` | Prefix each key with its namespace. |

Plus the [global flags](/commands/overview/#global-flags). These flags map to [settings](/configuration/schema/#settings-2) and override the values in `envx.yaml`.

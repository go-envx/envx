...
- A reveal-gated `diff` mode (`--reveal`/`--resolve`) that compares fully resolved values instead of declarations; `diff` intentionally stays at the declaration level (never resolves), so identical secret references and identical substitution formulas read as no difference. Revisit only if comparing effective values is wanted, noting it inverts today's contract (two different references that decrypt to the same plaintext would then show as no change) and would need a resolver, so it could fail on dangling references or unavailable keys.

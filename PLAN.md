...
12. ⬜ **Bulk encryption and decryption:** Implement selector-scoped, validate-before-write `Encrypt` and `Decrypt`; skip values already in the target state, reject explicit selectors that match nothing, and report changed identities without values.
13. ⬜ **Deletion:** Implement one-entry `Delete` without implicitly deleting a group keypair; defer group teardown until its retention semantics are designed.

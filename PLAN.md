...
12. ⬜ **Runtime reveal:** Decrypt ciphertext references lazily by required group, add masked/reveal policy for read commands, make `run` always reveal, and prove a decryption failure prevents child-process startup. This is the local-secrets MVP.
13. ⬜ **Bulk encryption and decryption:** Implement selector-scoped, validate-before-write `Encrypt` and `Decrypt`; skip values already in the target state, reject explicit selectors that match nothing, and report changed identities without values.
14. ⬜ **Keypair rotation:** Implement `Manager.RotateKeypair` and register `envx secrets rotate`. Validate and decrypt an entire group in memory, generate and re-encrypt with the replacement identity, enforce source/destination provenance rules, preserve file-backed rollback material, and document recovery from a second-write failure.

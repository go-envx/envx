...
func (m *ManifestLoader) Load() (*ManifestDoc, error) {
	path, err := m.discover()
	if err != nil {
		return nil, err
	}
...
// parse decodes raw YAML into a Manifest, runs structural validation, and
...
func (m *ManifestLoader) parse(data []byte) (*ManifestDoc, error) {
	var node yaml.Node

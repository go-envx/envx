...
type actionResult struct {
	...
	Summary env.ExplanationSummary
}
...
func execute(p actionParams, in *core.Input) (actionResult, error) {
	...
	explanation, err := resolved.Envmerge.Explain(env.ExplainParams{
		Key:    p.Key,
		Reveal: p.Reveal,
	})
	if err != nil {
...
func buildResult(
	explanation *env.Explanation, workspaceDir string, p actionParams,
) actionResult {
...
func shadowedFiles(
	origin env.Origin, workspaceDir string, absolute bool,
) []string {

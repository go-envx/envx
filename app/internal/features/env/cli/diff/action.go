...
func execute(p actionParams, in *core.Input) (actionResult, error) {
	...
	difference, err := resolved.Envmerge.Diff(env.DiffParams{
		EnvironmentA: p.EnvA,
		EnvironmentB: p.EnvB,
		Reveal:       p.Reveal,
	})
...
func buildResult(difference *env.DiffResult) actionResult {
	return actionResult{
...
func toChanges(in []env.Change) []actionResultChange {

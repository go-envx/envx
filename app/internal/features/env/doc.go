// Package env owns environment resolution: defaulting + validation of the
// target environment, namespace building, per-file flattening with layered
// overlay merge, and origin tracking. It
// consumes a plain env.Params, owns the resolved env.Settings the merge
...
// resolver state. The generic map helpers are unexported because env is
// their only consumer.
package env

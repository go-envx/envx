...
// consumes a plain envmerge.Params (built by the config package), owns the resolved
...
// behind Build and returns one immutable Result. The generic map helpers are
// unexported (map.go) because envmerge is their only consumer.
package envmerge
